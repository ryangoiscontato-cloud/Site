    ...
    <div>
      ...
      {produtos.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-12 flex flex-col items-center gap-3 text-gray-400">
          ...
          <p className="text-sm">Nenhum produto cadastrado. Toque em &ldquo;Novo Produto&rdquo; para começar.</p>
        </div>
      ) : (
        <>
          {/* ── Mobile: card list ── */}
          <div className="md:hidden space-y-3">
            {produtos.map(p => (
              <div key={p.id} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="px-4 pt-4 pb-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {p.categoria && (
                        <p className="text-sm text-gray-500 mt-0.5">{p.categoria}</p>
                      )}
                    </div>
                    <StatusBadge p={p} />
                  </div>
                ...
                {/* Card actions — full-width buttons, easy to tap */}
